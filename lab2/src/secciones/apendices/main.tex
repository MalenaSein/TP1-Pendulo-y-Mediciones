...
El análisis de la perturbación que introduce un voltímetro real con resistencia interna finita ($R_{\text{int}} \approx 10\text{ M}\Omega$) requiere evaluar matemáticamente el desvío de corriente en los nodos de la malla. 

Cuando el instrumento se conecta en paralelo con la resistencia $R_2$ para registrar su caída de potencial, la corriente total proveniente de la fuente se divide entre ambas ramas. Este acoplamiento modifica la resistencia efectiva de ese tramo del circuito, reduciéndola a un valor equivalente ($R'_2$) gobernado por la ecuación de paralelo:
\begin{equation}
    R'_2 = \frac{R_2 \cdot R_{\text{int}}}{R_2 + R_{\text{int}}}
\end{equation}

Al sustituir esta nueva resistencia equivalente en la expresión del divisor de tensión, el voltaje real ($V_{\text{real}}$) exhibido en el display del multímetro responde a:
\begin{equation}
    V_{\text{real}} = \frac{R'_2}{R_1 + R'_2} \times V_0
\end{equation}

A partir de este modelo matemático, se desglosan los dos escenarios experimentales analizados en el laboratorio:
...
Como consecuencia de este fenómeno físico (denominado efecto de carga), las caídas de tensión individuales medidas de forma sucesiva sobre el circuito de alta impedancia darán una suma menor al voltaje total entregado por la fuente ($V_{R1} + V_{R2} \neq V_0$). Es fundamental destacar que este resultado experimental no constituye una falla real ni una violación a la Segunda Ley de Kirchhoff, sino que es el reflejo matemático de haber conectado un instrumento real que alteró el estado del circuito durante el propio proceso de medición.
